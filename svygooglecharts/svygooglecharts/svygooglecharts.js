angular
		...
		function(googleChartApiPromise, $timeout) {
			return {
				...
				link: function($scope, $element, $attrs) {
					...
					/**
					 * Re-draws a chart with the new options provided
					 *
					 * 	elements.%%elementName%%.draw(options);
					 */
					$scope.api.draw = function(options) {
						if (typeof options === 'string') {
							options = JSON.parse(options);
						}
					...
					$scope.api.clearChart = function() {
						if ($scope.chartInternal.chart) {
							$scope.chartInternal.chart.clearChart();
						}
					}					
					...
					function drawChart() {
						...
						var chart = $scope.chartInternal && $scope.chartInternal.chart ? $scope.chartInternal.chart : null;
						$scope.chartInternal = { };
						if (chart) {
							$scope.chartInternal.chart = chart;
						}
						if ($scope.model.chartType || $scope.model.chartSpec.type) {
							$scope.chartInternal.type = $scope.model.chartSpec.type || $scope.model.chartType;
						}
    ...
    agcGstaticLoaderProvider
    	.setVersion('current') //this is the default
    	.addPackage('core')
  }
)